...
export default function PrivacyPushAllButton() {
  ...
  async function handleClick() {
    ...
    setError('');
    setResult(null);
    const res = await fetch('/api/admin/privacy-push-all', { method: 'POST' });
  ...
  return (
    <div className="mt-6 rounded-xl border border-orange-200 bg-orange-50 p-4 dark:border-orange-800/50 dark:bg-orange-900/10">
      <p className="mb-3 text-sm text-orange-800 dark:text-orange-300">
        <strong>Beheerdersactie:</strong> Vereis herbevestiging van de privacyverklaring voor alle gebruikers. De modal verschijnt bij hun volgende sessie.
      </p>
      <button
        onClick={handleClick}
        disabled={loading}
