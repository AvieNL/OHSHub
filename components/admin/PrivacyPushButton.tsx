...
export default function PrivacyPushButton({ userId, hasPending, pendingVersion, onDone }: Props) {
  ...
  if (hasPending) {
    return (
      <button
        onClick={handleClick}
        disabled={loading}
        title={`Herbevestigingsverzoek intrekken (v${pendingVersion})`}
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      title="Herbevestiging privacyverklaring verplichten"
