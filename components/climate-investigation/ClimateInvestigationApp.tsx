...
import {
  getAllClimateInvestigations,
  createClimateInvestigation,
  saveClimateInvestigation,
  deleteClimateInvestigation,
} from '@/lib/climate-investigation-storage';
import ClimateInvestigationShell from './ClimateInvestigationShell';
...
function exportAll(investigations: ClimateInvestigation[]) {
  const blob = new Blob([JSON.stringify(investigations, null, 2)], { type: 'application/json' });
  triggerDownload(blob, `ohshub-klimaat-onderzoeken-${new Date().toISOString().slice(0, 10)}.json`);
}

function exportOne(inv: ClimateInvestigation) {
  const blob = new Blob([JSON.stringify([inv], null, 2)], { type: 'application/json' });
  const slug = inv.name.replace(/\s+/g, '-').toLowerCase().slice(0, 40);
  triggerDownload(blob, `ohshub-klimaat-${slug}-${new Date().toISOString().slice(0, 10)}.json`);
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-700 dark:bg-zinc-900">
        <h2 className="mb-1 text-lg font-semibold text-zinc-900 dark:text-zinc-50">
          Nieuw klimaatonderzoek starten
        </h2>
        <p className="mb-5 text-sm text-zinc-500 dark:text-zinc-400">
          Geef het onderzoek een naam (bijv. &ldquo;Klimaat laslokaal zomer 2025&rdquo;). Opdrachtgever en locatie vult u in stap 2.
        </p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Bijv. Klimaat assemblagehal A — Q3 2025"
...
export default function ClimateInvestigationApp() {
  ...
  const handleUpdate = useCallback((updated: ClimateInvestigation) => {
    setInvestigations((prev) =>
      prev.map((i) => (i.id === updated.id ? updated : i)).sort(
        (a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime(),
      ),
    );
    saveClimateInvestigation(updated).catch(console.error);
  }, []);

  function handleCreate(name: string) {
    const inv = createClimateInvestigation(name);
    saveClimateInvestigation(inv).catch(console.error);
    setInvestigations((prev) =>
      [inv, ...prev].sort(
        (a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime(),
      ),
  ...
  function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        const arr: ClimateInvestigation[] = Array.isArray(parsed) ? parsed : [parsed];
        const valid = arr.filter(
          (item) => item && typeof item.id === 'string' && typeof item.name === 'string',
        );
        if (valid.length === 0) {
          setImportMsg({ type: 'err', text: 'Geen geldige klimaatonderzoeken gevonden.' });
          return;
        }
        await Promise.all(valid.map((inv) => saveClimateInvestigation(inv)));
        const refreshed = await getAllClimateInvestigations();
  ...
  function handleDelete(id: string) {
    deleteClimateInvestigation(id).catch(console.error);
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-zinc-300 py-16 dark:border-zinc-700">
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Nog geen klimaatonderzoeken</p>
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-xs text-zinc-400 dark:text-zinc-500">
                    ...
                    {inv.scenarios.length > 0 && (
                      <>
                        <span>·</span>
                        <span>{inv.scenarios.map((s) => SCENARIO_LABELS[s] ?? s).join(', ')}</span>
                      </>
                    )}
                    <span>·</span>
                    <span>Stap {inv.currentStep + 1} / 13</span>
                    <span>·</span>
