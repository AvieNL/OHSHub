...
function exportAll(investigations: Investigation[]) {
  const blob = new Blob([JSON.stringify(investigations, null, 2)], { type: 'application/json' });
  triggerDownload(blob, `ohshub-onderzoeken-${new Date().toISOString().slice(0, 10)}.json`);
}

function exportOne(inv: Investigation) {
  const blob = new Blob([JSON.stringify([inv], null, 2)], { type: 'application/json' });
  const slug = inv.name.replace(/\s+/g, '-').toLowerCase().slice(0, 40);
  triggerDownload(blob, `ohshub-${slug}-${new Date().toISOString().slice(0, 10)}.json`);
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <p className="mb-5 text-sm text-zinc-500 dark:text-zinc-400">
          Geef het onderzoek een naam zodat u het later terug kunt vinden. Onderzoeker en opdrachtgever vult u in stap 1.
        </p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Bijv. Oplosmiddelen lakkerij 2025"
...
export default function InvestigationApp() {
  ...
  const handleUpdate = useCallback((updated: Investigation) => {
    setInvestigations((prev) =>
      prev.map((i) => (i.id === updated.id ? updated : i)).sort(
        (a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime(),
      ),
    );
    saveInvestigation(updated).catch(console.error);
  }, []);

  function handleCreate(name: string) {
    const inv = createInvestigation(name);
    saveInvestigation(inv).catch(console.error);
    setInvestigations((prev) =>
      [inv, ...prev].sort(
        (a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime(),
      ),
  ...
  function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        const arr: Investigation[] = Array.isArray(parsed) ? parsed : [parsed];
        const valid = arr.filter(
          (item) => item && typeof item.id === 'string' && typeof item.name === 'string',
        );
        if (valid.length === 0) {
          setImportMsg({ type: 'err', text: 'Geen geldige onderzoeken gevonden in dit bestand.' });
          return;
        }
        await Promise.all(valid.map((inv) => saveInvestigation(inv)));
        const refreshed = await getAllInvestigations();
  ...
  function handleDelete(id: string) {
    deleteInvestigation(id).catch(console.error);
    setInvestigations((prev) => prev.filter((i) => i.id !== id));
    if (activeId === id) setActiveId(null);
    setDeleteConfirm(null);
  }

  // If an investigation is active, show the shell
  ...
  // Otherwise show the investigation list
  return (
    <>
      {showNewDialog && (
          ...
          <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-zinc-300 py-16 dark:border-zinc-700">
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                Nog geen onderzoeken
              </p>
              <p className="mt-1 text-xs text-zinc-400 dark:text-zinc-500">
                Klik op &ldquo;Nieuw onderzoek&rdquo; om te beginnen.
              </p>
            </div>
            <button
              onClick={() => setShowNewDialog(true)}
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-3 text-xs text-zinc-400 dark:text-zinc-500">
                    ...
                    <span>
                      {inv.substances.length} stof{inv.substances.length !== 1 ? 'fen' : ''}
                    </span>
                    <span>·</span>
