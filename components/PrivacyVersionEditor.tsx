...
interface Props {
  /** Current live body (to pre-fill the editor). */
  currentBody: string;
  /** Latest version number in DB, e.g. "1.2.0". Null when no versions exist yet. */
...
export default function PrivacyVersionEditor({ currentBody, latestVersion }: Props) {
  const isAdmin = useIsAdmin();
  const router = useRouter();

  const [open, setOpen] = useState(false);
  const [body, setBody] = useState(currentBody);
  // First-ever version defaults to major; subsequent versions default to patch
  ...
  async function handleSave() {
    setError('');
    setSaving(true);
    const res = await fetch('/api/admin/privacy-versions', {
      method: 'POST',
  ...
  return (
    <div className="mt-8 rounded-xl border border-orange-200 bg-orange-50 p-5 dark:border-orange-800/50 dark:bg-orange-900/10">
      <h3 className="mb-4 text-sm font-semibold text-orange-800 dark:text-orange-300">
        Nieuwe versie privacyverklaring
      </h3>
