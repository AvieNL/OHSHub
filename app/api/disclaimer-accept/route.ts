...
export async function POST() {
  ...
  const version = roleRow?.disclaimer_required_version;
  if (!version) {
  ...
  const { error } = await supabaseAdmin
    .from('user_roles')
    .update({
      disclaimer_version_accepted: version,
      disclaimer_accepted_at: new Date().toISOString(),
      disclaimer_required_version: null,
    })
    .eq('user_id', user.id);
  ...
  await supabaseAdmin.auth.admin.updateUserById(user.id, {
    user_metadata: { disclaimer_version_accepted: version },
  });
